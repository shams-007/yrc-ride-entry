import { useState } from "react";

// Geographically precise outline using YOUR exact raw, complete SVG path data
const BD_OUTLINE =
  "m25.212 83.944s-2.3768 1.6376-4.7535 1.6376c-2.3768 0-1.9806 2.4563-3.9613 1.6376-1.9806-0.8188-5.5458-3.6845-4.3574-5.7315 1.1884-2.0469 3.5651-3.2751 1.5845-7.369s2.7729 0 1.5845-4.5033-0.39614-2.0469 1.1884-5.7314 0-2.047 0.79225-6.5503c0.79225-4.5032 4.7535-9.8253 7.1303-9.8253s7.1303-0.40938 7.9225-2.8657c0.79225-2.4563-0.39615-6.5502 3.5651-9.8253 3.9613-3.2751 6.7342-1.2282 8.3186-2.8657c1.5845-1.6376 0.39615-3.2751-1.1884-5.322-1.5845-2.0469 0-3.6845-3.5651-4.0939-3.5651-0.40938-4.3574-4.0939-5.9419-0.81876s-2.3768 6.9596-3.5651 2.0469c-1.1884-4.9127-1.1884-4.9127 1.1884-8.5971 2.3768-3.6845 1.9806-9.0066 4.7535-8.5972 2.7729 0.40942 2.3768-1.6376 2.7729 2.047 0.39615 3.6845-0.39615 5.7314 3.169 6.9596 3.5651 1.2281 3.9613 2.4563 5.9419 4.5032s3.9613-1.6376 5.5458 0.81876c1.5845 2.4563 0.79225 4.0939 3.169 4.5033 2.3768 0.40938 2.3768 2.0469 4.3574 4.9127 1.9806 2.8657 3.169 0 5.1496 2.8657s0.39611 3.6845 4.7535 4.9127 7.1303-1.2282 9.1109 0.8188 0.39615 2.8657 5.5458 2.4563c5.1496-0.40934 8.3187-1.6376 3.5651-4.0939-4.7535-2.4564-4.3574-7.7784-7.5264-6.5502-3.169 1.2281-6.7342-3.2751-4.7535-4.9127 1.9806-1.6376 0.79226-3.2751 3.5651-2.8657 2.7729 0.40938 1.1884 2.4563 6.338 3.6845s2.7729-2.0469 5.1496 2.8657c2.3768 4.9127 5.5458 4.9127 3.9613 7.369-1.5845 2.4564-1.5845 0 0.39615 5.3221 1.9806 5.322-1.5845 6.9596 3.9613 9.4159s2.3768 2.4563 6.338 4.5033c3.9613 2.0469 9.9032 0.40938 14.261 3.6845 4.3574 3.2751 5.5458 1.2282 7.9225 3.2751 2.3768 2.0469 2.3768-4.5032 4.3574-4.9127 1.9806-0.40938 4.7535-2.4564 2.3768-4.5033-2.3768-2.0469-5.1496-2.8657-3.169-5.7315 1.9806-2.8657 3.9613-1.2281 4.3574-2.8657 0.39611-1.6376-1.5845-2.8657-0.39615-3.6845 1.1884-0.81876 3.5651-2.8657 3.9613-0.81876 0.39611 2.047-2.3768 4.9127 0.39611 5.7314 2.7729 0.8188 5.1496 1.2282 2.7729 0.8188-2.3768-0.40938 5.5458 4.0939 5.9419 5.7315 0.39611 1.6376-1.1884 5.7315 0.39611 6.9596 1.5845 1.2282 0.79226 0.81876 2.7729 2.4563 1.9806 1.6376-0.39614 4.9127 1.5845 4.9127s3.5651-1.2282 2.7729 0.40938c-0.79225 1.6376-3.5651 4.5033-3.5651 4.5033s-1.5845 2.4563-0.79225 6.5502 1.9806 9.8253 1.9806 13.1c0 3.2751 1.1884 17.194 0.79225 20.469-0.3961 3.2751-3.5651 11.872-1.9806 13.51 1.5845 1.6376 1.5845 4.5033 4.7535 2.4563 3.169-2.0469 3.9613-1.6376 7.1303-0.40938s3.169 3.6845 9.1109 3.6845 6.7341 2.4563 11.488 3.6845c4.7535 1.2282 9.507 2.8657 15.449 3.2751 5.5458 2.4563 1.1884 5.3221-1.5845 6.5502-2.7729 9.8253-3.5652-12.691-4.3574-8.5972-0.79224 4.0939 2.7729 13.1-1.9806 6.9596-4.7535-6.1408-9.507-6.5502-5.5458-9.8253 3.9613-3.2751 4.7535 0.81876 7.1303-2.8658 2.3767-3.6844 1.9806-5.7314-0.39622-11.463-2.3768-5.7315 1.5845-0.40942 0.39622-8.1878-1.1884-7.7783-1.5846-4.9126-1.5846-9.0065s3.169-10.235-0.79225-9.0066c-3.9613 1.2281-17.826 6.5502-18.618 8.1878-0.79224 1.6376-3.9612 2.8658-4.7535 7.7784-0.79225 4.9127 0 6.1408-1.5845 6.1408s-3.9613-2.4563-3.9613-9.0066c0-6.5502-5.5458-14.738-7.5264-19.651-1.9806-4.9127-15.053-18.013-15.449-20.469-0.39611-2.4564 0-12.282-0.39611-15.557-0.39615-3.2751-1.9806 3.6845-3.5651 6.9596s-3.169 0-6.7342 3.2751c-3.5651 3.2751-2.7729 6.5502-5.5458 8.5972-2.7729 2.0469-7.5264-2.4564-7.5264 0.40938 0 2.8657 4.3574 9.0066 4.3574 9.0066 0 3.6845 1.9806 5.7314-2.3768 6.9596s-6.338-0.40937-9.1109-1.2282c-2.7729-0.81876-3.169-7.369-4.7535-4.9127-1.5845 2.4563 1.5845 8.1878-5.5458 3.6845-7.1303-4.5032-11.488 9.4159-11.488 13.919 0 4.5032-0.79225 13.51-1.1884 19.241-0.39611 5.7314 1.5845 11.053-1.9806 15.147-3.5651 4.0939-7.9225 2.047-8.7148 2.8657-0.79225 0.81876-1.5845-6.5502-1.5845-6.5502s-4.7535 2.4564-3.9613 4.5033c0.79226 2.0469-5.5458-10.235-8.3186-6.9596-2.7729 3.2751-5.1496 9.0065-7.5264 6.9596-2.3768-2.047-3.169 4.5032-4.7535 6.9596-1.5845 2.4563-3.169 10.644-6.7342 12.691-3.5651 2.047-16.637 5.3221-16.241 3.2751 0.39613-2.0469 6.338-9.0065 1.9806-6.5502-4.3574 2.4564-5.5458-2.4563-5.5458-5.7315 0-3.2751 1.1884-4.5032-1.9806-4.0939-3.169 0.40939 1.5845 8.5972-3.5651 1.6376s-8.3187-11.053-8.3187-3.6845c0 7.369 2.7729 8.5972-1.9806 12.691s-7.5264 4.9127-9.1109 1.2281c-1.5845-3.6845-0.39611-3.6845-4.3574-3.6845s-5.1496 1.2282-2.7729 4.5033c2.3768 3.2751 6.338 5.7314 2.3768 6.1408-3.9612 0.40942-3.9612 1.2282-8.3186-2.0469s-11.092 2.8657-13.864 4.5032c-2.7729 1.6376-3.169 1.2282-4.7535-1.6376-1.5845-2.8657-4.3574-3.6844-5.1496-9.0065-0.79226-5.3221-2.7729-8.5972-0.79226-11.053 1.9806-2.4564-2.3768-11.463-3.169-7.7784-0.79225 3.6845-3.5651 2.8658-5.5458 6.1408-1.9806 3.2751-1.9806 2.047-1.9806 6.9596 0 4.9127 2.3768 0.81883 1.1884 7.7784s-0.39615 13.919-3.9613 10.235c-3.5652-3.6845-5.9419-18.013-7.9225-21.288-1.9806-3.2751-0.39611-4.0939 0-9.8254 0.3961-5.7314-2.7729-14.738 0-15.557 2.7729-0.81879-1.9806-10.644-4.7535-13.919-2.7729-3.2751-5.1496-0.8188-3.169-7.369s3.5651-3.2751 1.5845-9.0066c-1.9806-5.7314-5.9419-2.8657-3.9613-8.1878 1.9806-5.322-1.5845-19.329-0.79226-9.0065 0.79226-4.9127-5.1496-4.9127-1.9806-8.5972s9.9032-3.6845 5.9419-6.9596c-3.9613-3.2752-6.7341-10.235-6.7341-10.235s-0.79226 4.0939-1.9806-0.40938c-1.1884-4.5033 0 0 0.79226-6.5502 0.79225-6.5502-2.7729 1.2281 4.3574-8.5972s7.1303-10.235 0.79225-10.644c-6.338-0.40938-4.3574 6.9596-9.9032 3.2751s-10.695-7.7784-7.9225-8.5971c2.7729-0.81876 2.7729 0.40938 2.7729-2.4564 0-2.8657 0.79226-3.2751 1.1884-6.1408 0.39615-2.8657 1.9806-2.8657 2.3768-5.3221 0.39611-2.4563-5.9419-4.0939-8.3187-5.7314-2.3768-1.6376-1.9806-0.8188-6.7341-4.5033s-5.1496-2.8657-4.7535-8.5972c0.39611-5.7314-2.7729-5.7314 0.79225-8.1878 3.5651-2.4563 3.169-2.0469 6.7342-3.6845 3.5651-1.6376 7.9225-4.0939 7.9225-6.5502 0-2.4564-2.7729-2.8657-2.3768-6.1408 0.39615-3.2751-1.9806-8.5972-2.7729-8.5972-0.79225 0 3.5651 2.0469 3.5651-3.2751s0-6.1408-4.7535-7.369c-4.7535-1.2281-8.3187-1.2281-11.092-1.2281-2.7729 0-4.3574 0-9.9032-5.7315-5.5458-5.7314-5.5458-2.8657-13.072-4.0939-7.5264-1.2281-8.3187-3.2751-9.9032-9.0065s-8.7148-10.235-12.28-11.872c-3.5651-1.6376-3.5651-7.369-1.9806-7.369s4.3574-1.6376 5.5458-4.9127 4.3574-4.0939 4.3574-4.0939 3.169-2.4564 3.5651-6.1408c0.39615-3.6845 1.1884-12.691 3.5651-12.282 2.3768 0.40937 0.39615 7.369 3.9613 7.7784 3.5651 0.40938 11.488 3.2751 9.1109 0-2.3768-3.2751-5.1496-2.4563-0.79225-4.9127s11.092-12.282 8.7148-18.013c-2.3768-5.7314-1.1884-9.8253-1.1884-9.8253s2.7729-1.2282 7.1303 2.0469c4.3574 3.2752 1.5845 1.2282 7.5264 0.81881 5.9419-0.40938 8.3187 2.4563 13.072 0.81875 4.7535-1.6376 1.1884-5.322 5.5458-2.8657s5.1496 6.1408 6.7342 0c1.5845-6.1408 6.7342-6.1408 4.3574-7.7784-2.3768-1.6375-3.5651 2.4563-7.9225-1.2282s-5.9419-15.147-7.5264-17.194c-1.5845-2.0469-2.3768 1.2282-5.5458 1.2282s1.5845 1.6376-4.7535 1.6376-6.338-2.0469-9.507-2.8657c-3.169-0.81879-4.7535-0.81879-5.1496-2.4563-0.39611-1.6376-4.3574-2.8657-5.1496-6.1408-0.79225-3.2751-3.9613-5.322-4.7535-7.7784-0.79225-2.4564-0.39615-1.6376-3.5651-3.2751-3.169-1.6376-8.3187-8.5972-7.1303-7.7784z";

type MainBranch = {
  id: "dhaka" | "chittagong" | "sylhet";
  name: string;
  coord: string;
  cx: number;
  cy: number;
};

// Coordinates scaled to fit the 124x125 viewBox space
const MAIN: MainBranch[] = [
  { id: "dhaka", name: "🏍 DHAKA CENTRAL", coord: "Managed by Aminul Islam", cx: 64, cy: 69 },
  { id: "chittagong", name: "🏍 CHITTAGONG HUB", coord: "Led by Mohammad Sajid", cx: 91, cy: 96 },
  { id: "sylhet", name: "🏍 SYLHET REGIONAL", coord: "Coordinated by Farhana Akter", cx: 93, cy: 44 },
];

const DECOR = [
  { name: "Rajshahi", cx: 29, cy: 58, delay: 0 },
  { name: "Khulna", cx: 48, cy: 91, delay: 0.3 },
  { name: "Barisal", cx: 68, cy: 95, delay: 0.6 },
  { name: "Rangpur", cx: 43, cy: 28, delay: 0.9 },
  { name: "Mymensingh", cx: 67, cy: 45, delay: 1.2 },
  { name: "Comilla", cx: 89, cy: 76, delay: 1.5 },
  { name: "Cox's Bazar", cx: 100, cy: 114, delay: 1.8 },
  { name: "Jessore", cx: 42, cy: 81, delay: 0.15 },
];

// Trail aligned to your 124x125 grid
const BIKE_PATH =
  "M64,69 Q80,85 91,96 Q100,65 93,44 Q75,55 64,69";

export function BangladeshMap({
  height,
  label = "47 BRANCHES NATIONWIDE",
}: {
  height?: number | string;
  label?: string;
}) {
  const [hover, setHover] = useState<MainBranch["id"] | null>(null);

  return (
    <div
      className="relative w-full h-full overflow-visible animate-fade-in"
      style={{
        background: "transparent", // Clean borderless float
        height: height,
      }}
    >
      <style>{`
        @keyframes yrcMapDraw {
          from { stroke-dashoffset: 1; }
          to { stroke-dashoffset: 0; }
        }
        @keyframes yrcDotEnter {
          from { opacity: 0; transform: scale(0); }
          to { opacity: 1; transform: scale(1); }
        }
        @keyframes yrcDotPulse {
          0%, 100% { transform: scale(1); opacity: 1; }
          50% { transform: scale(1.3); opacity: 0.6; }
        }
        .yrc-bd-path {
          stroke-dasharray: 1;
          stroke-dashoffset: 1;
          animation: yrcMapDraw 2s ease-in-out forwards;
        }
        .yrc-bd-dot {
          opacity: 0;
          transform-box: fill-box;
          transform-origin: center;
          animation:
            yrcDotEnter 400ms ease forwards var(--enter-delay, 2s),
            yrcDotPulse 2s ease-in-out infinite var(--pulse-delay, 2.4s);
        }
        .yrc-bd-trail {
          stroke-dasharray: 4 4;
          opacity: 0;
          animation: yrcDotEnter 400ms ease forwards 2s;
        }
        .yrc-bd-bike {
          opacity: 0;
          animation: yrcDotEnter 400ms ease forwards 2.2s;
        }
      `}</style>

      <div className="relative flex h-full w-full flex-col">
        <div className="relative flex-1">
          {/* 
            We use -30 -15 184 155 as the padded coordinates.
            This shrinks the map, lifts it up perfectly, and aligns all tooltips correctly.
          */}
          <svg
            viewBox="-30 -15 184 155"
            width="100%"
            height="100%"
            preserveAspectRatio="xMidYMid meet"
            style={{ display: "block", maxHeight: "100%", overflow: "visible" }}
          >
            <defs>
              <path id="yrc-bike-motion-path" d={BIKE_PATH} />
              <filter id="yrc-blue-glow" x="-50%" y="-50%" width="200%" height="200%">
                <feGaussianBlur stdDeviation="3" result="blur" />
                <feMerge>
                  <feMergeNode in="blur" />
                  <feMergeNode in="SourceGraphic" />
                </feMerge>
              </filter>
            </defs>

            {/* Exactly render your precise, unrotated, un-truncated SVG path */}
            <path
              className="yrc-bd-path"
              d={BD_OUTLINE}
              pathLength={1}
              fill="#003087" // Official Yamaha Navy Blue website theme color
              stroke="#0047cc" // Glowing Yamaha Accent Blue border
              strokeWidth={0.5} // Beautiful crisp borders
              strokeLinejoin="round"
              style={{
                filter: "drop-shadow(0 0 1px rgba(0, 71, 204, 0.55))"
              }}
            />

            {/* Trail connecting three main cities */}
            <path
              className="yrc-bd-trail"
              d={BIKE_PATH}
              fill="none"
              stroke="#0047cc"
              strokeOpacity={0.5}
              strokeWidth={0.4}
            />

            {/* Decorative blue dots */}
            {DECOR.map((d, i) => (
              <g
                key={d.name}
                className="yrc-bd-dot"
                style={
                  {
                    ["--enter-delay" as string]: `${2 + (i + 3) * 0.1}s`,
                    ["--pulse-delay" as string]: `${2.4 + d.delay}s`,
                  } as React.CSSProperties
                }
              >
                <circle
                  cx={d.cx}
                  cy={d.cy}
                  r={0.8}
                  fill="#0047cc"
                  style={{ filter: "drop-shadow(0 0 1px rgba(0,71,204,0.7))" }}
                />
              </g>
            ))}

            {/* Motorcycle traveling the trail */}
            <g className="yrc-bd-bike" style={{ opacity: 0.75 }}>
              <g transform="translate(-1.5,-1.5)">
                <svg x="0" y="0" width="3" height="3" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="5.5" cy="16.5" r="3.5" />
                  <circle cx="18.5" cy="16.5" r="3.5" />
                  <path d="M5.5 16.5h6l3-6h-4" />
                  <path d="M14.5 10.5l4 6" />
                </svg>
                <animateMotion dur="8s" repeatCount="indefinite" rotate="auto">
                  <mpath href="#yrc-bike-motion-path" />
                </animateMotion>
              </g>
            </g>

            {/* Main red branch dots */}
            {MAIN.map((m, i) => (
              <g
                key={m.id}
                className="yrc-bd-dot"
                style={
                  {
                    ["--enter-delay" as string]: `${2 + i * 0.1}s`,
                    ["--pulse-delay" as string]: `${2.4 + i * 0.3}s`,
                    cursor: "pointer",
                  } as React.CSSProperties
                }
                onMouseEnter={() => setHover(m.id)}
                onMouseLeave={() => setHover((h) => (h === m.id ? null : h))}
              >
                <circle
                  cx={m.cx}
                  cy={m.cy}
                  r={1.3}
                  fill="#e60012"
                  stroke="#ffffff"
                  strokeWidth={0.15}
                  style={{ filter: "drop-shadow(0 0 3px rgba(230,0,18,0.9))" }}
                />
                {/* Larger invisible hit area */}
                <circle cx={m.cx} cy={m.cy} r={6} fill="transparent" />
              </g>
            ))}
          </svg>

          {/* Tooltips — positioned dynamically based on 124x125 grid percentages */}
          {MAIN.map((m) => {
            const active = hover === m.id;
            return (
              <div
                key={m.id}
                aria-hidden={!active}
                className="pointer-events-none absolute"
                style={{
                  left: `${((m.cx - (-30)) / 184) * 100}%`,
                  top: `${((m.cy - (-15)) / 155) * 100}%`,
                  transform: `translate(-50%, calc(-100% - 14px)) translateY(${active ? 0 : 5}px)`,
                  opacity: active ? 1 : 0,
                  transition: "opacity 200ms ease, transform 200ms ease",
                  zIndex: 50,
                }}
              >
                <div
                  className="rounded-lg bg-white font-sans"
                  style={{
                    padding: "8px 12px",
                    boxShadow: "0 8px 24px rgba(0,0,0,0.25)",
                    color: "#003087",
                    whiteSpace: "nowrap",
                    fontSize: 12,
                    lineHeight: 1.35,
                  }}
                >
                  <div style={{ fontWeight: 700 }}>{m.name}</div>
                  <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{m.coord}</div>
                  <a
                    href="/branches"
                    className="mt-1 inline-block font-semibold"
                    style={{ color: "#0047cc", fontSize: 11 }}
                  >
                    Visit Branch →
                  </a>
                </div>
              </div>
            );
          })}
        </div>

        {label ? (
          <div className="mt-3 flex flex-col items-center">
            <span
              className="font-display tracking-wider text-white/80"
              style={{ fontSize: 16, lineHeight: 1 }}
            >
              {label}
            </span>
            <span
              aria-hidden
              className="mt-2 block"
              style={{ width: 40, height: 2, backgroundColor: "#0047cc" }}
            />
          </div>
        ) : null}
      </div>
    </div>
  );
}
